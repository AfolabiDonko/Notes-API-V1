...
L'API doit être _state-less_. C'est à dire qu'elle ne nécessite pas l'usage de sessions.
...
- vérifier la validité du jeton JWT encodé en base 64 dans le _header_ HTTP `x-access-token` de chaque requête, puis en extraire l'identité de l'utilisateur.
...
Le jeton JWT de l'utilisateur connecté doit être fourni en base 64 dans le _header_ HTTP `x-access-token`.
